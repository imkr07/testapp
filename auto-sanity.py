...
# Function to generate a tabular report using string formatting
def generate_table(task_data):
    if not task_data:
        return "No data available to display in table."

    # Define headers for the table
    headers = ["Task Name", "Description", "Status", "Comments"]
    
    # Format the header row
    table = f"{headers[0]:<30} | {headers[1]:<40} | {headers[2]:<10} | {headers[3]:<30}\n"
    table += "-" * 120 + "\n"
    
    # Iterate through task_data and add rows to the table
    for task in task_data:
        row = f"{task['Task Name']:<30} | {task['Description']:<40} | {task['Status']:<10} | {task['Comments']:<30}"
        table += row + "\n"
